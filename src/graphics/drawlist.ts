...
import { Mat4, identityMat4 } from '../math/mat4';
import {
  BYTES_PER_COMPONENT,
  BYTES_PER_INSTANCE,
  BYTES_PER_VERTEX,
...
import { ARRAY_BUFFER, FLOAT, STATIC_DRAW, TRIANGLES, UNSIGNED_BYTE } from './glconstants';
import { colorAttrib, normalAttrib, positionAttrib, worldMatrixAttrib } from './programs';
...
export class DrawList {
  ...
  constructor(usage: number, geometry: Float32Array, maxInstances: number) {
    ...
    // Position attributes
    // One vertex position is [x, y, z], 3 components, starting at byte 0
    gl.enableVertexAttribArray(positionAttrib);
    gl.vertexAttribPointer(positionAttrib, 3, FLOAT, false, BYTES_PER_VERTEX, 0);

    // Normal attributes
    // One vertex normal is [nx, ny, nz], 3 components, starting at byte 12
    gl.enableVertexAttribArray(normalAttrib);
    gl.vertexAttribPointer(normalAttrib, 3, FLOAT, false, BYTES_PER_VERTEX, 3 * BYTES_PER_COMPONENT);
  ...
  addInstance(color: number): Mat4 {
    ...
    if (DEBUG) {
      if (i * COMPONENTS_PER_INSTANCE >= this.instanceData.length) {
        throw new Error(`Out of instances (i=${i}, max=${this.instanceData.length / COMPONENTS_PER_INSTANCE})`);
      }
    }
