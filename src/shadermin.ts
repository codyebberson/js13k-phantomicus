...
const SHADERS_DIR = './src/graphics/shaders/';
...
const VARIABLE_TYPES = ['bool', 'float', 'int', 'vec2', 'vec3', 'vec4', 'mat4'];
...
interface Variable {
  readonly tokenType: 'function' | 'var';
 ...
 * @returns
 */
export const buildShaders = (): void => {
  ...
  for (const file of files) {
    ...
    while (!parser.eof()) {
      ...
      if (EXTERN_ATTRIBUTES.includes(curr)) {
        if (parser.peek(-1) === '(' || parser.peek(-1) === ',') {
          console.log('TODO: handle function args');
          console.log(`curr="${curr}", prev="${parser.peek(-1)}", next="${parser.peek(1)}"`);
        }
        if (PRECISION_ATTRIBUTES.includes(parser.peek(1))) {
          const type = `${parser.peek(1)} ${parser.peek(2)}`;
          const name = parser.peek(3);
          globalVariables[name] = { tokenType: 'var', type: type, name: name, references: 0 };
          parser.index += 4;
        } else {
        ...
        if (name.match(/\w+/)) {
          file.localVariables[name] = {
            tokenType: parser.peek(2) === '(' ? 'function' : 'var',
            type: type,
            name: name,
            references: 0,
          };
  ...
  // Inline variables
  for (const file of files) {
    const localVariables = Object.values(file.localVariables);
    for (let i = localVariables.length - 1; i >= 0; i--) {
      ...
      if (variable.tokenType === 'var' && variable.references === 2) {
        const value = [];
        const parser = file.parser;
        parser.reset();
        while (!parser.eof()) {
        ...
        delete file.localVariables[variable.name];
      }
    }
  }
  ...
  for (const variable of Object.values(globalVariables).sort((a, b) => a.name.localeCompare(b.name))) {
    if (RENAME_ENABLED) {
  ...
  for (const file of files) {
    ...
    for (const variable of Object.values(file.localVariables).sort((a, b) => a.name.localeCompare(b.name))) {
      if (RENAME_ENABLED) {
        variable.rename = alphabet.charAt(nextVarIndex++);
      } else {
        variable.rename = variable.name;
  ...
  fs.writeFileSync('./src/graphics/shaders.ts', output.join(''));
};
